...
from .molecule_net import MyMoleculeNet as MoleculeNet
...
def load_pyg(name, dataset_dir):
    ...
    elif 'moleculenet' in name.lower():
        dataset_name = name[len('Moleculenet-'):]
        dataset = MoleculeNet(dataset_dir, dataset_name)
...
def load_dataset():
    ...
    dataset_dir = cfg.dataset.gwas_dir
...
def get_loader(dataset, sampler, batch_size, shuffle=True):
    ...
    if sampler == "full_batch" or len(dataset) > 1:
        loader_train = DataListLoader(dataset, batch_size=batch_size,
        ...
        loader_train = \
            ClusterLoader(dataset[0],
                          num_parts=cfg.train.train_parts,
                          save_dir="{}/{}".format(cfg.dataset.gwas_dir,
