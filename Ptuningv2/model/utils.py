...
from model.graphormer import (
    ...
    PrefixGraphormerForGraphPrediction
)
from model.light import (
    LiGhTForGraphPrediction,
    PrefixLiGhTForGraphPrediction
)
from model.gps import (
    GPSForGraphPrediction,
    PrefixGPSForGraphPrediction
)
...
PREFIX_MODELS = {
    ...
    "graphormer": {
        TaskType.GRAPH: PrefixGraphormerForGraphPrediction
    },
    "light": {
        TaskType.GRAPH: PrefixLiGhTForGraphPrediction
    },
    "gps": {
        TaskType.GRAPH: PrefixGPSForGraphPrediction
    }
}
...
AUTO_MODELS = {
    ...
    TaskType.GRAPH: {"graphormer": GraphormerForGraphPrediction,
                     "light": LiGhTForGraphPrediction,
                     "gps": GPSForGraphPrediction}
}
...
def get_model(model_args, task_type: TaskType, config: AutoConfig, fix_bert: bool = False):
    if model_args.prefix:
        ...
        if task_type == TaskType.GRAPH:
            model_class = AUTO_MODELS[task_type][config.model_type]
            model = model_class.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                revision=model_args.model_revision,
